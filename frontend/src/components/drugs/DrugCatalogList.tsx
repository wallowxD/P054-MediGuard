...
function DrugRow({ drug }: { drug: IDrugCatalogRow }) {
  ...
  return (
    <li className="border-b border-border py-3 last:border-b-0">
      <p className="text-sm font-medium text-foreground">{drug.brandName}</p>
 ...
 *
 * Từng dòng CHƯA phải link: `GET /api/v1/drugs/{id}` chưa có ở backend nên bấm vào sẽ dẫn
 * tới trang chi tiết rỗng. Khi endpoint đó lên, bọc nội dung dòng trong `<Link>` tới
 * `ROUTES.DRUG_INFORMATION/{id}` — không cần đổi gì khác.
 *
 * Dòng chỉ hiển thị dữ liệu định danh, không có nội dung lâm sàng: danh sách không kèm
 * trích dẫn nên theo luật số 1 nó không được phép nói bất cứ điều gì về công dụng hay
 * cảnh báo của thuốc.
 */
