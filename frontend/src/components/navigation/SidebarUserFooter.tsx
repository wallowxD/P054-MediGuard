...
interface SidebarUserFooterProps {
  ...
  onNavigate?: () => void;
  /** Sidebar desktop đang thu gọn: chỉ hiện avatar, tên/email giữ cho screen reader */
  collapsed?: boolean;
}
  ...
  return (
    <div className={`border-t border-border py-3 ${collapsed ? "px-2" : "px-3"}`}>
      <Link
        ...
        title={collapsed ? `Mở hồ sơ của ${name}` : undefined}
        className={`group flex items-center rounded-xl py-2 transition-colors hover:bg-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
