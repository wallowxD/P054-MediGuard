...
export default function DiseaseAutocomplete({
  selected,
  onChange,
  label = "Bệnh nền áp dụng cho lượt này",
  helpText = "Chỉ bệnh đã chọn từ danh mục mới được gửi để đối chiếu exact.",
  disabled = false,
}: {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 rounded-lg border border-border bg-input px-3 py-2 focus-within:ring-2 focus-within:ring-ring">
        ...
        <input id={id} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm trong danh mục bệnh nền…" disabled={disabled} className="min-w-0 flex-1 bg-transparent text-sm outline-none disabled:cursor-not-allowed disabled:opacity-50" />
      </div>
      {debounced && query.trim() === debounced ? (
        <div className="overflow-hidden rounded-lg border border-border bg-card" role="listbox">
          ...
          {data?.items.filter((item) => !selected.some((value) => value.id === item.id)).map((item) => (
            <button key={item.id} type="button" disabled={disabled} onClick={() => { onChange([...selected, item]); setQuery(""); }} className="block w-full px-3 py-2 text-left text-sm hover:bg-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50" role="option" aria-selected="false">
              {item.name}
            </button>
          ))}
      ...
      <div className="flex flex-wrap gap-2">
        {selected.map((item) => (
          <span key={item.id} className="inline-flex items-center gap-1.5 rounded-full bg-surface px-3 py-1 text-xs text-foreground-secondary">
            {item.name}
            <button type="button" disabled={disabled} onClick={() => onChange(selected.filter((value) => value.id !== item.id))} aria-label={`Bỏ ${item.name}`} className="rounded-full focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50"><X className="h-3.5 w-3.5" /></button>
          </span>
        ))}
      </div>
