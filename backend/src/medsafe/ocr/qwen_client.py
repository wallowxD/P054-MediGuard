...
class QwenVLClient:
    ...
    def __init__(
        ...
        self.api_key = (
            ...
            else getattr(settings, "qwen_api_key", getattr(settings, "dashscope_api_key", ""))
        )
        self.base_url = (
            base_url
            if base_url is not None
            else getattr(settings, "qwen_base_url", "https://dashscope-intl.aliyuncs.com/compatible-mode/v1")
        ).rstrip("/")
    ...
    def process_page_image(self, image_b64_uri: str, system_prompt: str | None = None) -> str:
        if not self.api_key:
            raise ValueError("QWEN_API_KEY / DASHSCOPE_API_KEY is not set. Please provide it in .env or settings.")
        ...
        while attempt < self.max_retries:
            attempt += 1
            try:
                logger.debug(f"Sending page request to Qwen API (attempt {attempt}/{self.max_retries})")
                ...
                logger.warning(f"Qwen API returned status {response.status_code}: {response.text}")
        ...
        raise RuntimeError(f"Failed to complete OCR request after {attempt} attempts. Last error: {last_error}")
