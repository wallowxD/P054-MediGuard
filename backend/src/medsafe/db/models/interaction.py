...
class DrugDrugInteraction(Base):
    ...
    version: Mapped[str | None] = mapped_column(String(50), server_default="v2")
...
class DrugDiseaseInteraction(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_d2dis_version", "version"),
    )
...
class Supplement(Base):
    """Bảng `supplements` — danh mục thực phẩm / TPCN."""
    ...
    supplement_name: Mapped[str] = mapped_column(Text, nullable=False, unique=True)
    supplement_name_unaccent: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    __table_args__ = (
        Index("idx_supplements_unaccent", "supplement_name_unaccent"),
        Index("idx_supplements_version", "version"),
    )

    def __repr__(self) -> str:
        return f"<Supplement id={self.id} name={self.supplement_name!r}>"


class DrugSupplementInteraction(Base):
    """Tương tác Thuốc – Thực phẩm / TPCN."""

    __tablename__ = "drug_supplement_interactions"
    ...
    supplement_id: Mapped[uuid.UUID | None] = mapped_column(
        postgresql.UUID(as_uuid=True), ForeignKey("supplements.id", ondelete="SET NULL")
    )
    supplement_name: Mapped[str] = mapped_column(Text, nullable=False)
    supplement_name_unaccent: Mapped[str] = mapped_column(Text, nullable=False)
    severity: Mapped[str] = mapped_column(String(50), server_default="moderate")
    effect_description: Mapped[str] = mapped_column(Text, nullable=False)
    management: Mapped[str | None] = mapped_column(Text)
    verbatim_quote: Mapped[str] = mapped_column(Text, nullable=False)

    source_type: Mapped[str] = mapped_column(String(50), server_default="leaflet_ocr")
    version: Mapped[str | None] = mapped_column(String(50), server_default="v2")
    review_status: Mapped[str | None] = mapped_column(String(50), server_default=REVIEW_STATUS_PENDING)
    reviewer_id: Mapped[uuid.UUID | None] = mapped_column(postgresql.UUID(as_uuid=True))
    reviewed_at: Mapped[datetime | None] = mapped_column(postgresql.TIMESTAMP(timezone=True))
    ...
    __table_args__ = (
        Index("idx_d2supp_ingredient", "canonical_ingredient"),
        Index("idx_d2supp_supplement_unaccent", "supplement_name_unaccent"),
        Index("idx_d2supp_version", "version"),
    )

    def __repr__(self) -> str:
        return f"<DrugSupplementInteraction {self.canonical_ingredient}+{self.supplement_name!r}>"
